...
describe('School', function () {
  ...
  it('cannot touch the internal representation of the data', function () {
    expect(school._roster).toBeUndefined();
    const keys = Object.keys(school);
    expect(school[keys[0]]).toBeUndefined();
    const ownKeys = Reflect.ownKeys(school);
    expect(school[ownKeys[0]]).toBeUndefined();
  });
});
